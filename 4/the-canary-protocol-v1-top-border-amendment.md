...
1. The **first line** of the response (FBD-CY5: Top Border)
2. Before all content, all board dialogue, all deliverables
3. Followed by a blank line

The Canary is the top border of every response — the instance writes context status before anything else. Rationale: LLM attention is highest at response start; postfix instructions degrade as output lengthens. Placing the Canary first makes it the structurally easiest element to produce, not the hardest.

If a response contains Pulse Lines, they appear at the end of the relevant content block as before. The Canary opens; Pulses close their respective sections.

**Short-form variant:** For sub-paragraph responses (under ~100 words), use zone-emoji shorthand: `[🟢 T4]` or `[🟡 T12]` or `[🔴 T18]`. Full format for all substantial responses.

### 3.2a — Retrospective Self-Check (FBD-CY7)

After writing the Canary line and before writing content, the instance checks: did my previous response include a Canary? If no, append a slip flag to the current Canary: `[Canary: ~XX% | ZONE | hint | T## | ⚠ prior slip]`. The flag is visible to the operator without being asked for. One-response-late detection is better than never.

If the current response is T1 (first response in session), no self-check is needed — there is no prior response.
...
| FBD-CY5 | Top Border | The Canary is the first line of every response. The instance writes context status before content. Placement at response start exploits peak attention — postfix placement fights attention degradation. The Canary is a border, not a footnote. |
| FBD-CY6 | Structural Salience | The Canary instruction appears in the founding prompt's Session Structure section — not only in this protocol document. Constitutional placement makes it identity-level, not protocol-level. |
| FBD-CY7 | Slip Detection | After writing the Canary, the instance checks whether its previous response included one. If not, a slip flag (`⚠ prior slip`) is appended to the current Canary line. One-response-late detection. The operator sees the slip without asking. |

---

## Section 5 — Sequence Position

**What comes before:** Nothing. The Canary is the first line of the response. It has no predecessor.

**What this does:** Provides persistent, passive context usage visibility to the operator.

**What comes after:** Everything. All content follows the Canary. The Canary opens; the response fills beneath it.
...
| v1 + Top Border Amendment | 14 April 2026 | Build Session | Placement flip: bottom → top (FBD-CY5). Founding prompt integration (FBD-CY6). Retrospective slip detection (FBD-CY7). Short-form variant for sub-paragraph responses. All-frame RCR: unanimous. Root cause: postfix instruction vs. attention gradient. |
